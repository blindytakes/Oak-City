...
export default function Nav() {
  ...
  return (
    <nav className="fixed top-0 w-full z-100 px-4 md:px-8 py-4 flex justify-between items-center backdrop-blur-xl bg-[rgba(250,250,247,0.85)] border-b border-border-light">
      <a href="#" className="flex items-center gap-2 no-underline" style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "1.3rem", letterSpacing: "-0.5px", color: "var(--color-text-primary)" }}>
      ...
      <ul className={`list-none flex gap-8 items-center md:flex ${menuOpen ? "!fixed !top-0 !right-0 !w-3/4 !max-w-[300px] !h-screen !bg-bg-white !flex-col !justify-center !gap-6 !p-8 !shadow-[-10px_0_40px_rgba(0,0,0,0.1)] !items-start" : "max-md:hidden"}`}>
        <li><a onClick={() => scrollTo("#services")} className="cursor-pointer text-text-secondary no-underline text-[0.9rem] font-medium hover:text-accent transition-colors">Services</a></li>
        <li><a onClick={() => scrollTo("#work")} className="cursor-pointer text-text-secondary no-underline text-[0.9rem] font-medium hover:text-accent transition-colors">Work</a></li>
        <li><a onClick={() => scrollTo("#pricing")} className="cursor-pointer text-text-secondary no-underline text-[0.9rem] font-medium hover:text-accent transition-colors">Pricing</a></li>
        <li>
          <a
